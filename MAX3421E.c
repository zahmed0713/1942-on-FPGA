...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	u8 send_buffer[2] = {(reg + 2),val};
	//u8 recieve_buffer[2] = {0,0};
	Status =  XSpi_Transfer(&SpiInstance, send_buffer, NULL, 2); //write reg + 2 via SPI
	//write val via SPI
	//read return code from SPI peripheral (see Xilinx examples) 
	if(Status != 0){ //if return code != 0 print an error
		xil_printf("\r\n error \r\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 write_buffer_data[nbytes+1];
	//u8 read_buffer_data[nbytes+1];
	write_buffer_data[0] = reg + 2; //write reg + 2 via SPI
	//read_buffer_data[0] = 0;
	for (int i = 1; i < nbytes+1; i++) {
			write_buffer_data[i] = data[i-1];
			//read_buffer_data[i] = 0;
		}
	Status =  XSpi_Transfer(&SpiInstance, write_buffer_data, NULL, nbytes+1);//write data[n] via SPI, where n goes from 0 to nbytes-1
...
BYTE MAXreg_rd(BYTE reg) {
	//psuedocode:
	XSpi_SetSlaveSelect(&SpiInstance, 1); //select MAX3421E (may not be necessary if you are using SPI peripheral)
	u8 send_buffer[2] = {reg,0};
	u8 recieve_buffer[2] = {0,0};
	Status =  XSpi_Transfer(&SpiInstance, send_buffer, recieve_buffer, 2);//write reg via SPI  //read val via SPI
	//consider making arrays buffers length two, since first spot of recieve may just return the reg
	//read return code from SPI peripheral 
	if(Status != 0){ //if return code != 0 print an error
		xil_printf("\r\n error \r\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0); //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	u8 val = recieve_buffer[1];
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 send_buffer[nbytes+1];
	send_buffer[0] = reg;
	u8 recieve_buffer[nbytes+1];
	Status =  XSpi_Transfer(&SpiInstance, send_buffer, recieve_buffer, nbytes+1);//write reg via SPI
	for(int i = 0; i < nbytes; i++){
		data[i] = recieve_buffer[i+1];
	}
